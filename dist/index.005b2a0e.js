...
module.bundle.HMR_BUNDLE_ID = "7395b77f005b2a0e";
"use strict";
...
},{}],"dLfVs":[function(require,module,exports) {
var _navbaranimation = require("./navbaranimation");
var _aboutmeanimation = require("./aboutmeanimation");
var _timelineanimation = require("./timelineanimation");
var _competencesanimation = require("./competencesanimation");
var _projectsanimation = require("./projectsanimation");
...
ctaButton.addEventListener('click', ()=>{
    _gsap.gsap.to(window, {
        duration: 0.5,
        scrollTo: ".introduction__content",
        ease: "ease-in-out"
    });
});

},{"./navbaranimation":"2BrXN","./aboutmeanimation":"6ZEM9","./timelineanimation":"bh3p4","./competencesanimation":"jMFhM","./projectsanimation":"frkl0","gsap":"iCVLt","gsap/ScrollToPlugin":"kBfnF"}],"2BrXN":[function(require,module,exports) {
var _gsap = require("gsap");
var _scrollToPlugin = require("gsap/ScrollToPlugin");
_gsap.gsap.registerPlugin(_scrollToPlugin.ScrollToPlugin);
const navbarButton = document.querySelector('.navbar__button');
const navbarButtonlines = document.querySelectorAll('.navbar__button-line');
const navbarContainer = document.querySelector('.navbar__list-container');
...
navLinks.forEach((l)=>{
    l.addEventListener('click', (event)=>{
        ...
        if (navbarOpen) toggleNav();
    });
});
const navBarTimeline = _gsap.gsap.timeline({
    defaults: {
        duration: 0.2,
        ease: "easeOut"
    },
...
navBarTimeline.set(".navbar__list-container", {
    visibility: "visible",
    left: "50%"
    ...
    opacity: 0,
    stagger: 0.02
}, "-=.3").from(".navbar__list-item-small", {
    y: "-150%",
    opacity: 0,
    stagger: 0.1
}, "-=0.30");
...
},{"gsap":"iCVLt","gsap/ScrollToPlugin":"kBfnF"}],"iCVLt":[function(require,module,exports) {
...
const introductionTimeline = _gsap.gsap.timeline({
    scrollTrigger: {
...
introductionTimeline.from('.introduction__header', {
    opacity: 0,
    y: "200%"
...
},{"gsap":"iCVLt","gsap/ScrollTrigger":"fkz0j"}],"fkz0j":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
_startup = 1, _proxies = [], _scrollers = [], _getTime = Date.now, _time1 = _getTime(), _lastScrollTime = 0, _enabled = 1, _passThrough = function _passThrough1(v) {
    return v;
}, _getTarget = function _getTarget1(t) {
...
}, _endAnimation = function _endAnimation1(animation, reversed, pause) {
    return animation && animation.progress(reversed ? 0 : 1) && pause && animation.pause();
}, _callback = function _callback1(self, func) {
...
}, _snapDirectional = function _snapDirectional1(snapIncrementOrArray) {
    var snap = gsap.utils.snap(snapIncrementOrArray), a = Array.isArray(snapIncrementOrArray) && snapIncrementOrArray.slice(0).sort(function(a1, b) {
        return a1 - b;
    });
    ...
    return value;
}, _createMarker = function _createMarker1(type, name, container, direction, _ref5, offset, matchWidthEl, containerAnimation) {
...
}, _clearScrollMemory = function _clearScrollMemory1() {
...
// },
_parsePosition = function _parsePosition1(value, trigger, scrollerSize, direction, scroll, marker, markerScroller, self, scrollerBounds, borderWidth, useFixedPosition, scrollerMax, containerAnimation) {
...
var ScrollTrigger = /*#__PURE__*/ function() {
    ...
    _proto.init = function init(vars, animation) {
        ...
        self.refresh = function(soft, force) {
            ...
            var size = getScrollerSize(), scrollerBounds = getScrollerOffsets(), max = containerAnimation ? containerAnimation.duration() : _maxScroll(scroller, direction), offset1 = 0, otherPinOffset = 0, parsedEnd = vars.end, parsedEndTrigger = vars.endTrigger || trigger, parsedStart = vars.start || (vars.start === 0 || !trigger ? 0 : pin ? "0 0" : "0 100%"), pinnedContainer = vars.pinnedContainer && _getTarget(vars.pinnedContainer), triggerIndex = trigger && Math.max(0, _triggers.indexOf(self)) || 0, i = triggerIndex, cs1, bounds, scroll, isVertical, override, curTrigger, curPin, oppositeScroll, initted, revertedPins;
            while(i--){
            ...
            end = Math.max(start, _parsePosition(parsedEnd || (parsedEndTrigger ? "100% 0" : max), parsedEndTrigger, size, direction, scrollFunc() + offset1, markerEnd, markerEndTrigger, self, scrollerBounds, borderWidth, useFixedPosition, max, containerAnimation)) || -0.001;
            ...
            while(i--){
                ...
                if (curPin && curTrigger.start - curTrigger._pinPush < start && !containerAnimation) {
                    ...
                    (curPin === trigger || curPin === pinnedContainer) && !_isNumber(parsedStart) && (offset1 += cs1); // numeric start values shouldn't be offset at all - treat them as absolute
            ...
            if (markerStart && offset1) {
                ...
                pinnedContainer && (cs1[direction.p] = "-=" + scrollFunc());
                gsap.set([
                    markerStart,
                    markerEnd
                ], cs1);
        ...
        self.update = function(reset, recordVelocity, forceFake) {
            ...
            var scroll = self.scroll(), p = reset ? 0 : (scroll - start) / change, clipped = p < 0 ? 0 : p > 1 ? 1 : p || 0, prevProgress1 = self.progress, isActive, wasActive, toggleState, action, stateChanged, toggled, isAtMax, isTakingAction;
            if (recordVelocity) {
                scroll2 = scroll1;
            ...
            if (clipped !== prevProgress1 && self.enabled) {
                ...
                if (pin) {
                    ...
                    if (!useFixedPosition) pinSetter(pinStart + pinChange * clipped);
                        ...
                        isAtMax = !reset && clipped > prevProgress1 && end + 1 > scroll && scroll + 1 >= _maxScroll(scroller, direction); // if it's at the VERY end of the page, don't switch away from position: fixed because it's pointless and it could cause a brief flash when the user scrolls back up (when it gets pinned again)
                        if (pinReparent) {
...
const lineSm = document.querySelector('.timeline__line--sm');
const lineBg = document.querySelector('.timeline__line--bg');
//get the svgelements of the current displayed timeline
if (getComputedStyle(lineSm).display === 'block') {
    lines = lineSm.querySelectorAll('[id^=tline]');
    circles = lineSm.querySelectorAll('[id^=tcirc]');
} else {
    lines = lineBg.querySelectorAll('[id^=tline]');
    circles = lineBg.querySelectorAll('[id^=tcirc]');
}
lines.forEach((line, i)=>{
    ...
    const tlLine = _gsap.gsap.timeline({
        scrollTrigger: {
            trigger: line,
            start: "top center",
            end: "90% center",
...
},{"gsap":"iCVLt","gsap/ScrollTrigger":"fkz0j"}],"jMFhM":[function(require,module,exports) {
var _gsap = require("gsap");
var _scrollTrigger = require("gsap/ScrollTrigger");
_gsap.gsap.registerPlugin(_scrollTrigger.ScrollTrigger);
//Competence section elements
const competenceCards = document.querySelectorAll('.competences__card');
...
},{"gsap":"iCVLt","gsap/ScrollTrigger":"fkz0j"}],"frkl0":[function(require,module,exports) {
var _gsap = require("gsap");
var _scrollTrigger = require("gsap/ScrollTrigger");
_gsap.gsap.registerPlugin(_scrollTrigger.ScrollTrigger);
//Get project cards
